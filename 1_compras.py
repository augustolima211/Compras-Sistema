# Conteúdo completo para 1_Pedidos_de_Compra.py
...
if 'pedido_final' not in st.session_state:
    st.session_state.pedido_final = pd.DataFrame(columns=['Código', 'Descrição', 'Quantidade', 'Fornecedor', 'Previsão de Entrega', 'Valor Unitário (R$)', 'Valor Total (R$)'])
...
def adicionar_item_pedido(codigo, descricao, quantidade, fornecedor, previsao, valor_unitario):
    st.session_state.pedido_final = st.session_state.pedido_final[st.session_state.pedido_final['Código'] != codigo]
    valor_total = quantidade * valor_unitario
    novo_item = pd.DataFrame({'Código': [codigo], 'Descrição': [descricao], 'Quantidade': [quantidade], 'Fornecedor': [fornecedor], 'Previsão de Entrega': [previsao], 'Valor Unitário (R$)': [f'{valor_unitario:,.2f}'], 'Valor Total (R$)': [f'{valor_total:,.2f}']})
...
with st.expander("Carregar/Substituir Planilha de Orçamento", expanded=(st.session_state.df_orcamento_original is None)):
    uploaded_file = st.file_uploader("Para iniciar ou substituir os dados, carregue uma nova planilha:", type="xlsx", key="uploader_pedidos")
    if uploaded_file:
        try:
            ...
            df_orcamento['Cod'] = df_orcamento['Cod'].astype(str).str.strip()

            st.session_state.df_orcamento_original = df_orcamento
            st.session_state.cotacoes_vencedoras = {}
...
if st.session_state.df_orcamento_original is not None:
    ...
    if itens_em_falta_df.empty:
        st.warning("✅ Nenhum item com a coluna 'Estoque' vazia foi encontrado nesta planilha.")
    else:
        ...
        if item_selecionado_display:
            ...
            fornecedor_padrao = ""
            unitario_str = str(dados_item['Unitario']).replace(',', '.') if pd.notna(dados_item['Unitario']) else "0.0"
            ...
            with col2:
                valor_unitario = st.number_input("Valor Unitário (R$)", min_value=0.0, value=float(valor_padrao), format="%.2f", key=f"val_{codigo_selecionado}")
    ...
    st.info("Aguardando o carregamento da planilha de orçamento para continuar.")
