...
# --- Funções Auxiliares (Nenhuma mudança aqui) ---
...
# --- NOVA ESTRUTURA DE UPLOAD ---
# O uploader fica em um expander. Ele começa aberto se nenhum dado foi carregado ainda.
with st.expander("Carregar/Substituir Planilha de Orçamento", expanded=(st.session_state.df_orcamento_original is None)):
    uploaded_file = st.file_uploader(
        "Para iniciar ou substituir os dados, carregue uma nova planilha:", 
        type="xlsx", 
    ...
    if uploaded_file:
        try:
            ...
            # Salva o dataframe na memória da sessão
            st.session_state.df_orcamento_original = df_orcamento
            # Limpa seleções antigas ao carregar nova planilha
            st.session_state.cotacoes_vencedoras = {}
            st.success("Planilha carregada com sucesso!")
            
            # Força o recarregamento do script para atualizar a interface
            st.rerun()
...
# Esta parte agora roda se os dados JÁ EXISTEM na memória, independentemente do uploader.
